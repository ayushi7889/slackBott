...
BAD_WORDS=['hmm','no','ummm']

SCHEDULED_MESSAGES=[
    {'text':'First message','post_at':(datetime.now() + timedelta(seconds=20)).timestamp(),'channel':'C01GB9KSY10'},
    {'text':'Second message!','post_at':(datetime.now() + timedelta(seconds=30)).timestamp(),'channel':'C01GB9KSY10'}
    ...
    def get_message(self):
        return{
            ...
            'blocks':[
                self.START_TEXT,
                self.DIVIDER,
                self._get_rection_task()

            ]
        }

    def  _get_rection_task(self):
...
if __name__=="__main__":
    ...
    list_scheduled_messages('C01GB9KSY10')
   # delete_scheduled_messages(ids,'C01GB9KSY10')
    app.run(debug=True)